...
def get_data(filters):
    ...
    select_fields = ["vl.start_date as start_date",
                    "v.full_name as volunteer", "v.gender as gender", "vl.nrcid as nrcid", "act.subject as activity", "vl.district_supported as district", 
                    "vl.club_position as club_position", "vl.club_type as club_type", "vl.club as club", 
                    "vl.days_of_voluntary_session AS days_spent", "vl.time_spent as total_hours"]

    if group_by_field:
        if group_by_field == "Volunteer":
            group_by_sql = "GROUP BY vl.volunteer"
            select_fields = ["v.full_name as volunteer", "sum(vl.days_of_voluntary_session) as days_spent", "sum(vl.time_spent) as total_hours"]
        elif group_by_field == "Gender":
            group_by_sql = "GROUP BY v.gender"
            select_fields = ["v.gender as gender", "sum(vl.days_of_voluntary_session) as days_spent", "sum(vl.time_spent) as total_hours"]
        elif group_by_field == "Club":
            group_by_sql = "GROUP BY vl.club"
            select_fields = ["vl.club as club", "sum(vl.days_of_voluntary_session) as days_spent", "sum(vl.time_spent) as total_hours"]
        elif group_by_field == "Club Type":
            group_by_sql = "GROUP BY vl.club_type"
            select_fields = ["vl.club_type as club_type", "sum(vl.days_of_voluntary_session) as days_spent", "sum(vl.time_spent) as total_hours"]
        elif group_by_field == "Activity":
            group_by_sql = "GROUP BY vl.activity_supported"
            select_fields = ["act.subject as activity", "sum(vl.days_of_voluntary_session) as days_spent", "sum(vl.time_spent) as total_hours"]
        elif group_by_field == "District":
            group_by_sql = "GROUP BY vl.district_supported"
            select_fields = ["vl.district_supported as district", "sum(vl.days_of_voluntary_session) as days_spent", "sum(vl.time_spent) as total_hours"]
...
def get_columns(filters):
    ...
    columns = [
			...
			{"label": _("Volunteer"), "fieldname": "volunteer", "fieldtype": "Link", "options": "Volunteer Profile", "width": 200},
